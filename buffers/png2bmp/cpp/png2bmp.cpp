...
class PngToBmpWorker : public AsyncWorker {
    public:
        PngToBmpWorker(Callback *callback, v8::Local<v8::Object> &pngBuffer, int size) 
            : AsyncWorker(callback) {
            unsigned char *buffer = (unsigned char*) node::Buffer::Data(pngBuffer);
            
            // We prefer to deal with STL containers rather than raw memory arrays, so we will create a vector from the pointer
        ...
        void HandleOKCallback () {
            Local<Object> bmpData = NewBuffer(
                (char *)bmp->data(), 
                bmp->size(), 
...
NAN_METHOD(SaveBMP) {
    ...
    Nan::Utf8String outputFileName(local_string);
    std::string outfile (*outputFileName);

    // Input buffer data (PNG)
    // This pointer does NOT point to data managed by V8 recall - it's on the normal C++ heap
    unsigned char* buffer = (unsigned char*) node::Buffer::Data(info[1]->ToObject(context).ToLocalChecked());
    unsigned int size = info[2]->Uint32Value(context).FromJust();

    // We prefer to deal with STL containers rather than raw memory arrays, so we will create a vector from the pointer
    std::vector<unsigned char> png_data(buffer, buffer + size);

    // vector where the bmp data will be stored
    std::vector<unsigned char> bmp;
...
NAN_METHOD(GetBMP) {
    ...
    unsigned char *buffer = (unsigned char*) node::Buffer::Data(info[0]->ToObject(context).ToLocalChecked());
    unsigned int size = info[1]->Uint32Value(context).FromJust();

    std::vector<unsigned char> png_data(buffer, buffer + size);

    // allocate the vector on the heap because we are building a buffer out of it's data to return to Node
    // and don't want to allow it to go out of scope until the buffer does (see buffer_delete_callback).
...
NAN_METHOD(GetBMPAsync) {
    ...
    v8::Local<v8::Object> pngBuffer = info[0]->ToObject(context).ToLocalChecked();
...
NAN_MODULE_INIT(Init) {
    ...
    Nan::Set(
        target, 
        New<String>("getBMP").ToLocalChecked(),
        GetFunction(New<FunctionTemplate>(GetBMP)).ToLocalChecked()
    );

    Nan::Set(
        target, 
        New<String>("getBMPAsync").ToLocalChecked(),
